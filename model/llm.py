...
class LLMManager:
    ...
    def mission_generator_daily(self, sub_categories: list[str]):
        # main_category를 sub_category에 따라 자동 지정


        for category in sub_categories:
            if category not in self._cs_categories + self._language_categories + self._tool_categories:
    ...
    def _determine_main_category(self, sub_category: str) -> str:
        if sub_category in self._cs_categories:
            return "CS"
        elif sub_category in self._language_categories:
            return "LANGUAGE"
    ...
    def mission_generator_free(self, sub_category: str) -> list[str]:
        # sub_category를 기반으로 main_category 결정
        main_category = self._determine_main_category(sub_category)

        # 프롬프트 생성
        prompt = self.generate_mission_prompt(main_category.lower(), sub_category)
